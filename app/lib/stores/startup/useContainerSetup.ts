...
const TEMPLATE_URL = '/template-snapshot-a4a51721.bin';
  ...
  useEffect(() => {
    const runSetup = async () => {
      try {
        ...
        console.error('Container setup error (new chat):', error);
        const errorMessage = error?.message || String(error);
        toast.error(`Failed to setup Chef environment: ${errorMessage}`, { duration: 10000 });
  ...
  useEffect(() => {
    ...
    const runSetup = async () => {
      try {
        ...
        console.error('Container setup error (existing chat):', error);
        const errorMessage = error?.message || String(error);
        toast.error(`Failed to setup Chef environment: ${errorMessage}`, { duration: 10000 });
        setContainerBootState(ContainerBootState.ERROR, error);
      }
...
async function setupContainer(
  ...
  console.log('📦 Starting container setup with snapshot:', options.snapshotUrl);
  ...
  console.log('✅ Snapshot decompressed');

  console.log('⏳ Waiting for WebContainer to boot...');
  const container = await webcontainer;
  console.log('✅ WebContainer booted, mounting filesystem...');
