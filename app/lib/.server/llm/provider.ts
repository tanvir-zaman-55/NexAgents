...
export type ModelProvider = Exclude<ProviderType, 'Unknown'> | 'OpenRouter';
type Provider = {
  maxTokens: number;
...
export function modelForProvider(provider: ModelProvider, modelChoice: string | undefined) {
  if (modelChoice) {
    ...
    if (modelChoice === 'openrouter-auto') {
      return 'z-ai/glm-4.6';
    }
  ...
  switch (provider) {
    ...
    case 'OpenRouter':
      return getEnv('OPENROUTER_MODEL') || 'z-ai/glm-4.6';
    default: {
  ...
  switch (modelProvider) {
    case 'Google': {
      ...
      if (userApiKey) {
        ...
        const vertexCredsJson = getEnv('GOOGLE_VERTEX_CREDENTIALS_JSON');
        const googleApiKey = getEnv('GOOGLE_API_KEY');

        if (vertexCredsJson) {
          const credentials = JSON.parse(vertexCredsJson);
          google = createVertex({
          ...
          throw new Error(
            JSON.stringify({ error: 'Either GOOGLE_API_KEY or GOOGLE_VERTEX_CREDENTIALS_JSON must be set' }),
          );
        }
      }
      provider = {
    ...
    case 'OpenRouter': {
      model = modelForProvider(modelProvider, modelChoice);
      const openrouterFetch = (url: RequestInfo | URL, init?: RequestInit) => {
        const headers = new Headers(init?.headers);
        headers.set('HTTP-Referer', 'https://chef.convex.dev');
        headers.set('X-Title', 'Chef AI');
        return (userApiKey ? userKeyApiFetch('OpenRouter') : fetch)(url, { ...init, headers });
      };
